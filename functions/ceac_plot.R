ce_plot <- function(ce, plot, wtp_line=20000,vline=NULL){
  ktop <- 100000 
  icea.dt <-  cea(ce, k = seq(0, ktop, 500), sim = "sim", strategy = "strategy",
                   grp="grp", e = "QALY", c = "cost", sample = "sim")
  icea.pw.dt <-  cea_pw(ce,  k = seq(0, ktop, 500), comparator = "NE", sample = "sim",
  ...
  if(plot==1){
   p <-  ggplot(icea.pw.dt$delta, aes(x = ie, y = ic, col = factor(strategy))) + 
      geom_point(size = .2) + 
  ...
  if(plot==2){
    if(is.null(vline)){
      p <- ggplot(icea.dt$mce, aes(x = k, y = prob, col = factor(strategy))) +
